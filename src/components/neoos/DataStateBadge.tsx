...
export function DataStateBadge() {
  ...
  const rawState = deriveDataState({ report, source, loadError, now: now ?? undefined });

  /**
   * Once a real position exists, the fixture report is gated off every surface,
   * so a "DEMO" badge would be warning about content that is not on screen. It
   * becomes "Not analysed" instead: the accurate description of the state, and
   * the one that does not train people to ignore badges.
   */
  const hasUserPortfolio = useActivePortfolio().hasUserPortfolio;
  const state: DataState =
    rawState === "demo" && hasUserPortfolio ? "insufficient_evidence" : rawState;
