...
export default function PortfolioPage() {
  ...
  return (
    <div className="grid gap-3.5">
      {/*
        Declared holdings sit OUTSIDE the gate. They are the subject's own data
        and must survive whether or not an analysed report exists — gating them
          ...
          >
            {holdings.length === 0 ? (
              <p className="rounded-xl border border-[#222d36] bg-panel2 p-4 text-xs text-muted">
                No holdings match the current report&apos;s assets. Reset to
                demo data to see the reference portfolio.
              </p>
            ) : (
                    ...
                    <details className="group overflow-hidden rounded-[14px] border border-[#222d36] bg-panel2">
                      ...
                      <div className="border-t border-[#222d36] p-3.5">
                        <dl className="grid grid-cols-2 gap-x-3 gap-y-2 text-[11px] sm:grid-cols-3">
                          <div>
                            <dt className="microlabel text-[8px]">
                              Allocation
                            </dt>
                          ...
                          <div>
                            ...
                            <dd className="mt-0.5 font-semibold">
                              {valuationProvenance(engine, asset.id)
                                ? formatRange(
                                    asset.intrinsicValueLow,
                                    asset.intrinsicValueHigh,
                                  )
                          ...
                          <div>
                            ...
                            <dd className="mt-0.5 font-semibold">
                              {asset.confidence}% ·{" "}
                              {confidenceTier(asset.confidence)}
                            </dd>
                          </div>
                          <div>
                            <dt className="microlabel text-[8px]">
                        ...
                        {asset.status === "insufficient_evidence" ? (
                          <div className="mt-3 rounded-xl border border-amber/40 bg-amber/10 p-3">
                            <p className="microlabel text-[8px] text-amber">
                              Insufficient evidence
                            </p>
                            <ul className="mt-1.5 space-y-1 text-[11px] leading-relaxed text-[#ffe1c2]">
                              {(asset.insufficientReasons ?? []).map(
                                (reason) => (
                        ...
                        <p className="mt-3 text-[11px] text-[#8e9aa5]">
                          <span className="microlabel mr-1.5 text-[8px]">
                            Next review
                          </span>
